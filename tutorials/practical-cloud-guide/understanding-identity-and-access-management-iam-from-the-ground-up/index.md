...
Identities provide access to the AWS account and to associated services and resources.  We’ve already talked about the AWS account user has access to all resources. An IAM user, or simply user, is a person or application that has specific permissions to access resources. Users have long term credentials, such as passwords and access keys, that grant permanent access to specified resources. 
...
```
The account id is 123456789101 and your users can login at an address which takes the form of `https://<account id>.signin.aws.amazon.com/console`, e.g., `https://123456789101.signin.aws.amazon.com/console`. If you login as either user, you will see that you can't access any resources.

**Step 2**: We’ll need a resource to manage. We’ll create an S3 bucket with several folders containing files. Note that S3 buckets require that you choose unique name.
...
**Step 3**: Now that we have users, a group, and an S3 bucket set up, we can create our first policy. Our first policy allows listing all the buckets at the root of the account.
...
The S3 will display all the buckets that belong to the master account including the bucket we created. Note that the bucket is marked with insufficient permissio5, and if you choose the bucket and try to open it you will see the message `Insufficient permissions to list objects`.

![Listing of S3 buckets in the account](./images/s3-1.png)

**Step 4**: We can update the group policy to let all our users to list the folders and objects at the root of our bucket. We’ll add this statement to our policy. Note that the resource is the bucket we created and we've added a condition to allow listing the objects in the bucket. 
