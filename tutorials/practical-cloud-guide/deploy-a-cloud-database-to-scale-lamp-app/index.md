...
In previous tutorials we [deployed a single application on a VPC](/tutorials/practical-cloud-guide/deploy-a-java-application-on-linux). Applications like these are called monolithic applications because all the components are tightly coupled in a single server. Cloud architectures are frequently loosely coupled with application components connected via the network. This tutorial is an update of a [Lightsail workshop](https://www.lightsailworkshop.com/).
