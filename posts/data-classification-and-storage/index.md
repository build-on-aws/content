...
There are many tools and technologies that help us utilize data within these three categories. For example, we use Relational Database Management Systems, Online Analytical Processing (OLAP), and Online Transactional Processing  (OLTP) to work with structured data. For unstructured data, we’ll organize and store it using NoSQL databases and AI-driven tools. And we rely on delimited files (CSV, TSV), HTML, XML, and JSON to help us format and utilize semi-structured data.
