...
While human expertise remains vital, the application of Generative AI, specifically Large Language Models (LLMs) like [OpenAI'sGPT](https://openai.com/gpt-4), [Anthropic Claude](https://www.anthropic.com/index/claude-2), or [Amazon Titan](https://aws.amazon.com/bedrock/titan), can support and expedite this complex decision-making process. 
...
In this post, we will explore how Generic AI can be used to facilitate architectural decisions. We'll dive into the capabilities of LLMs, the significance of crafting effective prompts, and the limitations of relying on AI for decision-making. By the end, you'll have a good understanding of how you can effectively steer the LLM so that it helps you gather and understand requirements, provides information to evaluate options, and ultimately assists you in making well-informed decisions.
...
Chances are you'll hear one of our most infamous lines: "It depends." 
...
Looking at the response, we can see that, while the AI already tries to be helpful, listing some generic pros and cons of the options, it ultimately gave me the same response: "The best option depends [...]". And I must admit that I have said this *way* too often myself (sorry). Sure, it's true, but it also is obvious. And frustrating. 
...
Consider the following scenario: We're a small business and want to develop an application that handles customer data. The core functions will include recording and updating customer details, searching for customer data, and recording interactions with customers. It should be Java-based and provide a RESTful API to manage HTTP requests from web and mobile apps.
...
By creating context-specific prompts tailored to the specific situation, we can extract much more valuable insights from the AI.
...
4. **Iterate and refine:** Effective prompts may require refinement through iterations. Assess the AI responses and adjust your prompts accordingly to gather more targeted and valuable information.

By following these guidelines, you'll extract the maximum value from the AI and ensure that the prompts lead the toward addressing the specific considerations of the decision at hand.
...
Before diving into the interaction with the LLM, we need to lay the groundwork. We create  a starting point for the converastion by defining the LLM's role and setting the context.
...
You don't provide theoretic or generic information. Your responses will consider our specific use-case. You don't jump to conclusion but ask clarifying questions.
...
Before we can make any architectural decision, it is crucial to understand the requirements and constraints of our system. AI language models can be extremely useful in refining these requirements. We can interact with them in a conversational manner, revealing crucial details and considerations.
...
Here, the AI asks some interesting questions, some of which me might not even have thought of ourselves. This alone creates a lot of value, because it helps us dive deep and think about the specifics of our application.

### Iterate and Refine

We may not have answers to all of the questions yet, but can start answering them as good as we can. Once that's done, we can ask it to dive even deeper by asking more questions, by simply repeating the following phrase at the end of the prompt: "Please continue asking clarifying questions. Don't summarize or conclude anything. Ask the questions in a numbered list.
...
```text
We're done with question. Please create a list of requirements. Keep it brief.
```
...
To make a well-informed decision, we need to compare the available options in the context of our requirements. Let's continue the conversation with the following prompt:
...
**⚠ Warning:** While LLMs tend to come across in a very convincing manner, it is absolutely possible that the information they provide is outdated or even wrong. Due to the nature of how LLMs work, they do not "understand" what they are talking about, so any stated "fact" should be double-checked using reliable sources. Nevertheless, when using the techniques in this post, you can get a quick understanding of the most important aspects of your system. Even if you still have to do some additional research, you'll have a lot of information pointing you in the right direction.

## Discovering Other Options

In the, usually quite busy, world of software development, many decisions are multifaceted and complex. We rely on our knowledge and experience to guide our choices and may overlook a subtle challenge: the unknown unknowns. These are the options, risks, or solutions that we might not even be aware of.
...
I've spared you the details in the screenshot, but the model dilligently listed some options, like [AWS Outposts](https://aws.amazon.com/outposts) for on-premises deployment, that clearly weren't a good fit for our application.

However, it also recommended [AWS App Runner](https://aws.amazon.com/apprunner), a fully managed container service that allows us to deploy containers without all the operational overhead in a self-managed environment. This could in fact be a viable alternative that we haven't even considered!

## Making a Decision

Now, that we have defined the LLM's role, defined the context, gather all the requirements, and even discovered a third option, we can use the AI to help us with the actual decision-making.
...
Whenever you make an architectural decision, I recommend documenting the process. While the *what* and *how* often remains visible over the lifetime of an application, the *why we chose this*, including *why we didn't chose the alternative*, may get lost over time. 
...
The future is promising, and I hope that with this guide, you are ready to add AI to your toolkit.