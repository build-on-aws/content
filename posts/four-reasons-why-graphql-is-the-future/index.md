...
description: It's been a long time coming, but the cracks of REST API design are starting to show in an age where everyone needs things to work faster and cheaper while being kind to the planet. GraphQL offers a whole new approach and here are four reasons why it is primed to become the norm not the exception in a not-so-distant future. 
tags:
  - api design
  - REST APIs
  - GraphQL
...
# Four Reasons Why GraphQL Is The Future

REST architecture has revolutioned the industry and powered much of the innovation of the last decades including microservice architecture which rose to prominence and rapidly became the new de-facto modern Service-Oriented Architecture (SOA) pattern. 

In fact, distributed and SOA grew to such degree that a typical modern application has a dependency on tens, hundreds of even thousands of microservices which are composed together to deliver rich user experiences. This leads to a few frontend and backend challenges.
...
It's brilliant! This relatively simple approach knocks quite a few birds in one go: it solves current discoverability challenges, it maintains a clean separation between the frontend and backend allowing them to optmized and changed with a high degree of independence from each other, and, finally, it simplifies and reduces the amount of configuration data and logic for consumers.
...
Application users are becoming so accustomed to real-time experiences that it is fast becoming the norm. In this day and age, even non-tech-savvy people complain about loading times and spinners. People are used now to immediate feedback and the trend is only going to intensify over the years which places GraphQL in a leading position for future proofing your APIs with that kind of capability built in.

## Conclusion

Much like any of its ancestors, GraphQL will continue to mature and evolve as the community embraces it. It has already gone from first public release in 2015 to mainstream adoption in hyper-scale production systems such as PayPal, GitHub, Instagram and many others in a couple of years and it only keeps on growing as the community learns more about it and more businesses keep proving its potential. There is great appetite for reducing data waste and finding sustainable options for running systsems without compromising on features and GraphQL is fits perfectly as a core piece of this new wave of application design.
