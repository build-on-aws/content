...
Finally, we add a trigger to the Lambda function. For this solution, it will be an [EventBridge (CloudWatch Events) trigger](https://docs.aws.amazon.com/eventbridge/latest/userguide/eb-run-lambda-schedule.html), and we will use a rate expression to trigger the function every 13 days.
